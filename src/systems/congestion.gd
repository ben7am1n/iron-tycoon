## Congestion — CORE-LAYER INTEGRATION STUB (Story SL-002).
##
...
## Congestion story lands. It keeps the system's public contract surface
...
## restores them. The real Congestion story (prev buffer + per-cell smoothed
## + recomputed access_reachable) replaces this file.
...
## Two-phase init (ADR-0001). Registers the Congestion RNG sub-stream exactly
