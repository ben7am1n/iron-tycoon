## Economy — CORE-LAYER INTEGRATION STUB (Story SL-002).
##
...
## Economy story lands. It keeps the system's public contract surface
...
## serialize() captures {counter, balance, rng_state}, deserialize() validates
## and restores them. The real Economy story (balance + revenue/costs)
## replaces this file.
...
## Stub state: the money balance — the one gameplay value AC3's QA pin calls
## out explicitly ("balance — all unchanged" after a failed load).
var balance: int = 0


## Two-phase init (ADR-0001). Registers the Economy RNG sub-stream exactly
...
## Balance is deliberately NOT auto-incremented here — the stub leaves it to
## the test/owner to set, matching the real system's data-driven revenue.
...
## Pure read — no draws, no counter/balance mutation (SL-001 AC1 counts
## serialize calls, so serialize stays side-effect free).
