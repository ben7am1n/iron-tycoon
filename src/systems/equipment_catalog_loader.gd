...
## as validation failures (AC-E.1). Since Story 007: the pipeline also
## validates the cost boundary (cost >= 0, AC-E.2) and unlock_requirement is
## stored as an OPAQUE string with no existence check (AC-E.3); a JSON entry
## that omits "cost" gets the provisional formula value at EquipmentDef
## construction (EC-006 contract). Remaining semantic ranges
## (use_duration_* ranges) are Story 005 and still NOT implemented here.
...
##
## Extension point: Story 005 (use-duration) appends its validator here IN
## THIS FIXED ORDER — do not reorder (determinism contract). Cost (Story 007,
## AC-E.2) is validated LAST so the QA case "returns 3 errors (FOOTPRINT_EMPTY,
## ACCESS_COUNT, COST_NEGATIVE)" holds its documented order.
...
static func _load_single_definition(entry: Variant) -> Dictionary:
	...
	# COST_NEGATIVE) holds.
	var validation_failures := _validate_all(normalized["footprint"], normalized["access"], effective_cost)
