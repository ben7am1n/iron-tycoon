...
##   结构层 GAMEPLAY（前台）→ 会员中景 → 设备前景 → 结构层 FOREGROUND（立柱/
##   吊灯，V3 §4 可轻微遮挡）→ 环境前景（大植物）→ 放置幽灵（活动决策预览
##   优先级最高）。
##
...
func _draw() -> void:
	...
	if _grid_visible:
		_draw_grid_lines()
	# 2.5D 空间层级（art-bible-25d §1 + V3 §4）：环境背景 → 结构层 GAMEPLAY
	# （前台）→ 会员中景 → 设备前景 → 结构层 FOREGROUND（立柱/吊灯）→
	# 环境前景（大植物可轻微遮挡）→ 幽灵（活动决策预览优先级最高）。
	if _structure_art != null:
