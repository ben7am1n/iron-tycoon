# tests/evidence/light_smooth_capture.gd
#
# Deterministic before/after evidence for the smooth-light-pool refactor.
# Run in windowed mode from the repository root:
#   godot --path . res://tests/evidence/light_smooth_capture.tscn -- before
#   godot --path . res://tests/evidence/light_smooth_capture.tscn -- after
extends Node

const MAIN_SCENE := preload("res://src/main.tscn")
const CAPTURE_FRAME := 48
const CLOSEUP_RECT := Rect2i(120, 105, 1040, 500)
...
func _process(_delta: float) -> void:
	...
	if image == null:
		push_error("light smooth capture: viewport image is null")
		get_tree().quit(1)
		return
	var path := "res://tests/evidence/light-smooth-%s.png" % _label
	var err := image.save_png(ProjectSettings.globalize_path(path))
	if err == OK and _label == "after":
		_save_after_details(image)
	get_tree().quit(0 if err == OK else 1)


func _save_after_details(after: Image) -> void:
	var crop_rect := CLOSEUP_RECT.intersection(Rect2i(Vector2i.ZERO, after.get_size()))
	var closeup := after.get_region(crop_rect)
	closeup.save_png(ProjectSettings.globalize_path(
		"res://tests/evidence/light-smooth-after-closeup.png"))

	var before := Image.new()
	if before.load("res://tests/evidence/light-smooth-before.png") != OK:
		push_warning("light smooth capture: before frame missing; comparison skipped")
		return
	if before.get_size() != after.get_size():
		push_warning("light smooth capture: frame sizes differ; comparison skipped")
		return
	...
	compare.save_png(ProjectSettings.globalize_path(
		"res://tests/evidence/light-smooth-compare.png"))

	var lighting := _main.get_node_or_null("WorldViewport/WorldRoot/LightingLayer")
	if lighting != null:
		var light_map: Image = lighting.call("light_map_image")
		light_map.save_png(ProjectSettings.globalize_path(
			"res://tests/evidence/light-smooth-lightmap.png"))
