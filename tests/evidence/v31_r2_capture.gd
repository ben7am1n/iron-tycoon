...
## / yoga_mat(9,2)）—— 证据采样依赖这些设备的场景语义（treadmill console
## cyan / belt 高对比履带 / bike 飞轮）。main.gd 初始布局是经济默认（空房
## 开局），证据脚本不依赖它 —— 已放置则跳过，未放置则走 PlacementSystem
## 完整拖放链补齐（同 main.gd _drag_drop / phase3_capture）。
...
func _on_placed(instance_id: int, equipment_id: String, _footprint_cells: Array) -> void:
	if _main.has_method("_on_placed"):
		_main.call("_on_placed", instance_id, equipment_id, _footprint_cells)
