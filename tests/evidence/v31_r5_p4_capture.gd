...
#
# 输出：
#   tests/evidence/v31-r5-p4-ui.png          —— 渲染帧（主场景视口 1280×720，
#                                               会员在场：8 会员确定性注入）
#   tests/evidence/v31-r5-p4-ui-hud-zoom.png —— HUD 特写（顶栏挂牌 + 底部
#                                               展示架，2× NEAREST 放大）
#
# 用法（窗口模式——headless 用 dummy 渲染驱动，get_image() 返回 null，
# 4.7.1 已验证；窗口捕获是项目既有证据方法）：
#   godot --path . res://tests/evidence/v31_r5_p4_capture.tscn
#
...
const OUT_PATH := "res://tests/evidence/v31-r5-p4-ui.png"
const ZOOM_PATH := "res://tests/evidence/v31-r5-p4-ui-hud-zoom.png"
...
func _verify_hud(img: Image) -> void:
	...
	# 5) V3.1 返工5 P4 FAIL#1：顶带外轮廓锯齿 —— 顶缘非水平直线（2-3px+ 抖动）。
	...
	# 6) V3.1 返工5 P4 FAIL#1：四角不齐 —— 顶带内容包围盒四角不应是完整
	...
	# 7) V3.1 返工5 P4 FAIL#2：顶部世界/HUD 交界线打散 —— 挂牌下方墙色带
	...
	# 8) V3.1 返工5 P4 FAIL#2：底部世界/HUD 交界线打散 —— 架条上方墙色带
...
## V3.1 返工5 P4：顶带 HUD 材质包围盒（限 [0..max_y) 行 —— 排除底部交界
...
## V3.1 返工5 P4：扫描 [y0..y1) 各行，返回最长「墙色带」连续 run（px）。
