# tests/evidence/v31_r6_p1_capture.gd — V3.1 返工5 P3 证据捕获（N4/N11 色彩层次）
#
# 渲染真实主场景（src/main.tscn，V3.1 P1 oblique + P2 设备 + P3 手绘密度 +
# P4 pixel lighting + P5 高饱和焦点全链路，含返工5 P3 背景烘焙化 + 远景墙暖化）
# 并保存：
#   - tests/evidence/v31-r6-p1-space.png   全场景帧（8 会员在场注入保留）
#   - tests/evidence/v31-r6-p1-closeup.png    北墙/远景墙 2.5x 特写
#     （N11 远景墙暖化 WALL_BASE_FAR 786F6E r-b=10 + N4 手绘墙面语言 /
#      无纯色平涂带：烘焙纹理笔触 + 延展墙区手绘填充）
#   - tests/evidence/v31-r6-lightmap.png      LightingLayer 静态 light map
#     （qa_v31r4_independent.py / qa_v31r4p3_independent.py 独立复核输入）
#   - tests/evidence/v31-r6-projected-lightmap.png  投影空间灯泡→落点光束
#
...
#   godot --path . res://tests/evidence/v31_r6_p1_capture.tscn
extends Node
...
const OUT_FULL := "res://tests/evidence/v31-r6-p1-space.png"
const OUT_CLOSEUP := "res://tests/evidence/v31-r6-p1-closeup.png"
const LIGHTMAP_PATH := "res://tests/evidence/v31-r6-lightmap.png"
const PROJECTED_LIGHTMAP_PATH := "res://tests/evidence/v31-r6-projected-lightmap.png"
...
## 特写变换：聚焦设备带（treadmill(2,2)(6,3) / bike(2,5) 区域，世界
## 中心 ≈ (128,128)）—— 覆盖轮廓勾边/色阶分层/噪点退让/焦点区（R6 P1
## 证据要求）。scale 2.5 → 世界 (426/2.5, 240/2.5)=(170,96) 可见；
## position 使 (128,128) 居中。
const CLOSEUP_SCALE := Vector2(2.5, 2.5)
const CLOSEUP_FOCUS := Vector2(128, 128)   # 世界坐标焦点（设备带中心）
...
func _ready() -> void:
	...
	if _world_root == null:
		push_error("v31_r6_p1_capture: WorldRoot not found")
		get_tree().quit(1)
...
func _export_light_maps() -> void:
	var lighting := _main.get_node_or_null("WorldViewport/WorldRoot/LightingLayer")
	if lighting == null:
		push_error("v31_r6_p1_capture: LightingLayer not found")
		return
...
func _grab() -> Image:
	...
	if img == null:
		push_error("v31_r6_p1_capture: get_image() returned null (headless dummy driver?)")
		get_tree().quit(1)
		return null
	return img
...
func _save(img: Image, out_path: String) -> void:
	...
	if err != OK:
		push_error("v31_r6_p1_capture: save_png failed err=%d path=%s" % [err, abs_path])
		get_tree().quit(1)
		return
...
func _verify_world_frame(img: Image) -> void:
	...
	# V3.1 返工6 P1 内嵌验证（非正式验收；gate PIL 为准）：
	_verify_r6_noise_focus(img)


## V3.1 返工6 P1（FAIL1/FAIL2 自测）：
##   a. 设备体零噪点：treadmill(2,2) 顶面 / bike(2,5) 顶面采样窗口的
##      4-bit 色阶多样性应显著低于 R5（R5 实测 treadmill=27 / bike=34
##      桶）。设备顶面合法包含 5 层设计色 + 灯下暖池（bike 骑在池落点
##      上，R4 ring 硬门要求池保持 hash 散射 —— 桶数不能为 0；目标是
##      「散射噪点移除 + jitter 降密」带来的净下降）。
##      阈值：treadmill ≤ 21（R5 27 → 目标 -22%）；bike ≤ 34（池主导，
##      R4 ring 约束下基本持平即算通过；GPT 视觉自检为最终判定）。
##   b. 焦点区：设备带（世界 64..256, 64..192）平均明度应高于同材质
##      远离设备的地板（QA r4p1 的 focal > far_zone 口径，同一投影）。
func _verify_r6_noise_focus(img: Image) -> void:
	# (a) 设备顶面噪点（世界像素 → 屏幕，全场景变换 z=设备高度）：
	#   treadmill(2,2) top @(96,80,z=30)；bike(2,5) top @(96,176,z=36)
	var treadmill_buckets := _sample_bucket_count(img, _world_to_screen_full_z(Vector2(96, 80), 30.0), 16)
	var bike_buckets := _sample_bucket_count(img, _world_to_screen_full_z(Vector2(96, 176), 36.0), 16)
	# 返工6 目标：设备顶面 4-bit 桶数显著低于 R5（R5 实测 27/34 桶）
	_ok(treadmill_buckets <= 21,
		"R6 equipment top low noise (treadmill buckets %d <= 21, 设备体零噪点)" % treadmill_buckets)
	_ok(bike_buckets <= 34,
		"R6 equipment top low noise (bike buckets %d <= 34, 池落点受 R4 ring 约束)" % bike_buckets)
	# (b) 焦点区 vs 周边（同 QA r4p1 口径：世界像素明度）
	...
	_ok(focal_lum > far_zone_lum,
		"R6 focus zone brighter than periphery (focal lum %.3f > far %.3f, 焦点区)" % [focal_lum, far_zone_lum])


## 采样窗口内 4-bit 色桶数（噪点密度代理：桶越多 = 高频颗粒越多）。
func _sample_bucket_count(img: Image, center: Vector2i, radius: int) -> int:
	var buckets := {}
...
func _verify_closeup(img: Image) -> void:
	# V3.1 返工6 P1：特写聚焦设备带 —— 验证轮廓/色阶/噪点退让：
