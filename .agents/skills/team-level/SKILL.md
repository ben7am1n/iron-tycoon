...
user-invocable: true
---
<!-- Generated from .claude/skills/team-level/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, run shell commands, delegate work, ask structured questions, track plans.
- Requested quality tier: balanced; reasoning effort: medium.


When this skill is invoked:

**Decision Points:** At each step transition, use structured user-question capability to present
...
Use the native subagent or delegation capability to spawn each team member as a subagent:
...
**Gate**: Use structured user-question capability to present all three Step 1 outputs (narrative brief, lore foundation, visual direction targets) and confirm before proceeding to Step 2.
...
Use structured user-question capability with options:
...
**Gate**: Use structured user-question capability to present Step 2 layout (including any unresolved adjacent area dependencies) and confirm before proceeding to Step 3.
...
**Gate**: Use structured user-question capability to present Step 3 outputs and confirm before proceeding to Step 4.
...
**Gate**: Use structured user-question capability to present both Step 4 results. If the accessibility-specialist returned any BLOCKING concerns, highlight them prominently and offer:
...
After all subagent outputs are collected, spawn `level-designer` via native subagent or delegation capability to compile and write the final document:
...
to sub-agents spawned via native subagent or delegation capability. Each sub-agent enforces the "May I write to [path]?"
...
If any spawned agent (via native subagent or delegation capability) returns BLOCKED, errors, or cannot complete:

1. **Surface immediately**: Report "[AgentName]: BLOCKED — [reason]" to the user before continuing to dependent phases
2. **Assess dependencies**: Check whether the blocked agent's output is required by subsequent phases. If yes, do not proceed past that dependency point without user input.
3. **Offer options** via structured user-question capability with choices:
