...
user-invocable: true
---
<!-- Generated from .claude/skills/hotfix/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
Confirm with structured user-question capability:
...
If the check passes, use structured user-question capability before creating the branch:
...
Use the native subagent or delegation capability to request sign-off in parallel:
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` via native subagent or delegation capability with:
