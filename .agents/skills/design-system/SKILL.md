...
user-invocable: true
---
<!-- Generated from .claude/skills/design-system/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, delegate work, ask structured questions, track plans.
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use structured user-question capability:
...
Use structured user-question capability:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   structured user-question capability for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use structured user-question capability to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   structured user-question capability. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use structured user-question capability with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use structured user-question capability:
...
spawn `creative-director` via native subagent or delegation capability:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents via native subagent or delegation capability in parallel:
...
- Surface any disagreements between agents to the user via structured user-question capability
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents via native subagent or delegation capability in parallel:
...
- Present the specialists' proposals to the user for review via structured user-question capability
...
For required systems: **spawn `art-director` via native subagent or delegation capability** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` via native subagent or delegation capability using gate **CD-GDD-ALIGN** (`.claude/docs/director-gates.md`).
...
**When delegating via native subagent or delegation capability**:
...
- The main session presents the agent's output to the user via structured user-question capability
- The user decides; the main session writes to file
...
2. **structured user-question capability** at every decision point (Explain -> Capture pattern):
