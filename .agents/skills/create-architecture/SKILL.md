...
user-invocable: true
---
<!-- Generated from .claude/skills/create-architecture/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, run shell commands, ask structured questions, delegate work.
...
Display a one-paragraph summary of what the document will contain (layers, modules, data flows, ADR gaps). Then use structured user-question capability:
...
**Step 2 — Spawn `lead-programmer` via native subagent or delegation capability using gate LP-FEASIBILITY (`.claude/docs/director-gates.md`):**
...
Use structured user-question capability — "Technical Director and Lead Programmer have reviewed the architecture. How would you like to proceed?"
...
Show the proposed Document Status block inline, then use structured user-question capability:
