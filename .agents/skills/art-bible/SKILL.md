...
user-invocable: true
---
<!-- Generated from .claude/skills/art-bible/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->

## Platform Contract

- Invoke named skills with the current host's native skill syntax.
- Use the current host's native subagent or delegation capability for named roles.
- Preserve every user approval and write-permission gate in this workflow.
- Allowed capability classes: read files, discover files, search text, modify files, delegate work, ask structured questions.
- Requested quality tier: balanced; reasoning effort: medium.
...
Use structured user-question capability with two tabs:
...
**Agent delegation (MANDATORY)**: Spawn `art-director` via native subagent or delegation capability:
...
Present the art-director's draft to the user. Use structured user-question capability:
...
**Agent delegation**: Spawn `art-director` via native subagent or delegation capability with Visual Identity Statement and mood targets. Ask: "Define the shape language for this game. Connect each shape principle back to the visual identity statement and a specific game pillar. Explain what these shape choices communicate to the player emotionally."
...
**Agent delegation**: Spawn `art-director` via native subagent or delegation capability with Visual Identity Statement and mood targets. Ask: "Design the color system for this game. Every semantic color assignment must be explained — why does this color mean danger/safety/reward in this world? Identify which color pairs might fail colorblind players and specify what backup cues are needed."
...
**Agent delegation**: Spawn `art-director` via native subagent or delegation capability with sections 1–4. Ask: "Define character design direction for this game. Cover: visual archetype for the player character (if any), distinguishing feature rules per character type (how do players tell enemies/NPCs/allies apart at a glance?), expression/pose style targets (stiff/expressive/realistic/exaggerated), and LOD philosophy (how much detail is preserved at game camera distance?)."

Write the approved section to file.

### Section 6: Environment Design Language

**Agent delegation**: Spawn `art-director` via native subagent or delegation capability with sections 1–4. Ask: "Define the environment design language for this game. Cover: architectural style and its relationship to the world's culture/history, texture philosophy (painted vs. PBR vs. stylized — why this choice for this game?), prop density rules (sparse/dense — what drives the choice per area type?), and environmental storytelling guidelines (what visual details should tell the story without text?)."
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — use structured user-question capability to let the user decide.
...
After all sections are complete (or the scoped set from Phase 1 is complete), spawn `creative-director` via native subagent or delegation capability using gate **AD-ART-BIBLE** (`.claude/docs/director-gates.md`).
...
Use structured user-question capability for next steps. Only include options that are genuinely next based on the state check above:
