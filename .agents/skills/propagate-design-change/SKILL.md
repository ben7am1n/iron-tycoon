...
user-invocable: true
---
<!-- Generated from .claude/skills/propagate-design-change/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, run shell commands, delegate work.
...
Spawn `technical-director` via native subagent or delegation capability using gate **TD-CHANGE-IMPACT** (`.claude/docs/director-gates.md`).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; use structured user-question capability with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
