...
user-invocable: true
---
<!-- Generated from .claude/skills/regression-suite/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- `/regression-suite report` — read-only status report (no writes); suitable
  for sprint reviews
- No argument — if a sprint is clearly active (sprint plan exists with in-progress stories), run `update`. If ambiguous or no active sprint is detected, use structured user-question capability:
