...
user-invocable: true
---
<!-- Generated from .claude/skills/code-review/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, run shell commands, delegate work, ask structured questions.
...
Spawn all applicable specialists simultaneously via native subagent or delegation capability — do not wait for one before starting the next.
...
For Logic and Integration stories, also spawn `qa-tester` via native subagent or delegation capability in parallel with the engine specialists. Pass:
