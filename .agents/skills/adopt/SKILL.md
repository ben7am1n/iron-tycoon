...
user-invocable: true
---
<!-- Generated from .claude/skills/adopt/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
**Audit mode:** `invocation argument 0` (blank = `full`)
...
If the project appears fresh (no artifacts at all), use structured user-question capability:
...
and offer to handle it immediately using structured user-question capability. Choose the first
branch that applies:
