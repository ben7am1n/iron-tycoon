...
user-invocable: true
---
<!-- Generated from .claude/skills/story-readiness/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, ask structured questions, delegate work.
...
**Scope:** `invocation argument 0` (blank = ask user via structured user-question capability)
...
If no argument is given, use structured user-question capability:
...
Spawn `qa-lead` via native subagent or delegation capability using gate **QL-STORY-READY** (`.claude/docs/director-gates.md`).
...
- **GAPS [list]** → surface the specific gaps to the user via structured user-question capability:
