...
user-invocable: true
---
<!-- Generated from .claude/skills/dev-story/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, run shell commands, delegate work, ask structured questions.
...
If they differ, use structured user-question capability before proceeding:
...
specialist to spawn via native subagent or delegation capability.
...
Spawn the chosen programmer agent(s) via native subagent or delegation capability with the full context package:
...
- **File writes are delegated** — all source code, test files, and evidence docs are written by sub-agents spawned via native subagent or delegation capability. Each sub-agent enforces the "May I write to [path]?" protocol individually. This orchestrator does not write files directly.
