...
user-invocable: true
---
<!-- Generated from .claude/skills/brainstorm/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, search the web, delegate work, ask structured questions.
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use structured user-question capability** at key decision points throughout brainstorming:
   ...
   structured user-question capability to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an structured user-question capability with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use structured user-question capability for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab structured user-question capability with these exact tab labels:
...
Present all three. Then use structured user-question capability to capture the selection.

**CRITICAL**: This MUST be a plain list call — no tabs, no form fields. Use exactly this structure:

```
structured user-question capability(
...
Ask these as structured user-question capability calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use structured user-question capability:
...
If the user selects B, C, or D, make the revision, then use structured user-question capability again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via native subagent or delegation capability in parallel before moving to Phase 5. Issue both Task calls simultaneously — do not wait for one before starting the other.**
...
Collect both verdicts, then present them together using a two-tab structured user-question capability:
...
- **Target platform**: Use structured user-question capability — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use structured user-question capability — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` via native subagent or delegation capability using gate TD-FEASIBILITY (`.claude/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via native subagent or delegation capability using gate PR-SCOPE (`.claude/docs/director-gates.md`).**
...
5. Use structured user-question capability for write approval:
...
If [B]: ask which section to revise using structured user-question capability with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use structured user-question capability — "Ready to write the updated concept document?"
