...
user-invocable: true
---
<!-- Generated from .claude/skills/story-done/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, run shell commands, modify files, ask structured questions, delegate work.
...
3. If multiple in-progress stories are found, use structured user-question capability:
...
### Manual verification with confirmation (use structured user-question capability)
...
Batch up to 4 manual verification questions into a single structured user-question capability call:
...
1. Ask: is there a test — unit, integration, or confirmed manual playtest — that
   ...
   - **Manual confirmation**: if the criterion was verified via structured user-question capability
...
After completing the deviation checks in Phase 4, spawn `qa-lead` via native subagent or delegation capability using gate **QL-TEST-COVERAGE** (`.claude/docs/director-gates.md`).
...
- `lean` → use structured user-question capability before proceeding:
...
Spawn `lead-programmer` via native subagent or delegation capability using gate **LP-CODE-REVIEW** (`.claude/docs/director-gates.md`).

Pass: implementation file paths, story file path, relevant GDD section, governing ADR.

Present the verdict to the user. If CONCERNS, surface them via structured user-question capability:
...
Use structured user-question capability before writing anything:
...
- Use structured user-question capability for the code review prompt and for batching manual
