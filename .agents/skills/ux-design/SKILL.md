...
user-invocable: true
---
<!-- Generated from .claude/skills/ux-design/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, ask structured questions, delegate work.
- Requested quality tier: balanced; reasoning effort: medium.
- Preferred stable role: ux-designer; use native delegation or role loading.
...
**If no argument is provided**, do not fail — ask instead. Use structured user-question capability:
...
2. **Questions**: Ask what is needed to draft this section. Use structured user-question capability
...
3. **Options**: Where design choices exist, present 2-4 approaches with pros/cons.
   Explain reasoning in conversation, then use structured user-question capability to capture the decision.
4. **Decision**: User picks an approach or provides custom direction.
5. **Draft**: Write the section content in conversation for review. Flag provisional
   assumptions explicitly.
6. **Approval**: Use structured user-question capability:
...
7. **Write**: Use structured user-question capability: "May I write the [section name] section to `[filepath]`?"
...
- Use structured user-question capability to capture the choice:
...
- Use structured user-question capability: "Want an ASCII wireframe as part of this spec?"
...
Use structured user-question capability to confirm:
...
Use structured user-question capability to step through items in groups of 3-4, not all at once.
...
Work through patterns in groups. Use structured user-question capability:
...
Then use structured user-question capability:
...
When delegating to another agent via the native subagent or delegation capability:
