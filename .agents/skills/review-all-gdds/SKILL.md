...
user-invocable: true
---
<!-- Generated from .claude/skills/review-all-gdds/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
**Focus:** `invocation argument 0` (blank = `full`)
...
Use structured user-question capability for write permission:
- Prompt: "May I write this review to `design/gdd/gdd-cross-review-[date].md`?"
- Options: `[A] Yes — write the report` / `[B] No — skip`

If any GDDs are flagged for revision, use a second structured user-question capability:
...
After all file writes are complete, use structured user-question capability for a closing widget.
