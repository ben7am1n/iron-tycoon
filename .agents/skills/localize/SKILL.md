...
user-invocable: true
---
<!-- Generated from .claude/skills/localize/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Preferred stable role: localization-lead; use native delegation or role loading.
...
Spawn `localization-lead` via native subagent or delegation capability. Ask them to audit the following for cultural sensitivity across the target locales (read from `assets/data/strings/` and `assets/`):
...
Spawn `localization-lead` via native subagent or delegation capability with:
