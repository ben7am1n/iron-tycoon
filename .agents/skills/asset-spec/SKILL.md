...
user-invocable: true
---
<!-- Generated from .claude/skills/asset-spec/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- If it exists: read it, find the first entity or screen with status "Needed" but no spec file yet, and use structured user-question capability:
...
Present the full proposed inventory to the user in conversation. Then use structured user-question capability:
...
- **system**: Read `design/gdd/[target-name].md`. Extract the **Visual/Audio Requirements** section. If it doesn't exist or reads `[To be designed]`:
  ...
  Use structured user-question capability: `[A] Describe needs manually` / `[B] Stop — complete the GDD first`
...
- **character** or **entity**: Read `design/narrative/characters/[target-name].md` or search `design/narrative/` and `design/assets/entity-inventory.md` for a matching entry. Extract visual description, role, and any specified distinguishing features.
  - **If no source doc exists**: do not fail. Instead, use structured user-question capability:
...
Present the full identified list to the user. Use structured user-question capability:
...
**`art-director`** via native subagent or delegation capability:
...
**`technical-artist`** via native subagent or delegation capability:
...
After presenting all specs, use structured user-question capability:
