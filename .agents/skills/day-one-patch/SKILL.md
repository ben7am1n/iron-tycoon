...
user-invocable: true
---
<!-- Generated from .claude/skills/day-one-patch/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
Use structured user-question capability to confirm proceeding or reduce scope.
...
Spawn `release-manager` via native subagent or delegation capability. Ask them to produce a rollback plan covering:
...
1. Spawn `lead-programmer` via native subagent or delegation capability with:
...
3. Spawn `qa-tester` via native subagent or delegation capability to verify: does the bug reproduce after the fix?
...
Spawn `qa-lead` via native subagent or delegation capability with:
