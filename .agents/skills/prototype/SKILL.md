...
user-invocable: true
---
<!-- Generated from .claude/skills/prototype/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Preferred stable role: prototyper; use native delegation or role loading.
- Isolation preference: use a host-native isolated worktree when available and preserve approval gates.
...
Otherwise, use structured user-question capability to confirm intent before proceeding:
...
Assess which path best fits the hypothesis, then use structured user-question capability with your
...
- `full` → spawn `creative-director` via native subagent or delegation capability using gate **CD-PLAYTEST** if
...
2. **Choose path** — same structured user-question capability widget as Phase 3 (HTML / Engine / Paper).
