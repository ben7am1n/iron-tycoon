...
user-invocable: true
---
<!-- Generated from .claude/skills/create-stories/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Preferred stable role: lead-programmer; use native delegation or role loading.
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via native subagent or delegation capability using gate **QL-STORY-READY** (`.claude/docs/director-gates.md`).
...
Use structured user-question capability to close with context-aware next steps:
