...
user-invocable: true
---
<!-- Generated from .claude/skills/design-review/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
Use structured user-question capability for ALL closing interactions. Never plain text.
...
Work through all blocking items, asking for design decisions only where you cannot resolve the issue from the GDD and existing docs alone. Group all design-decision questions into a single multi-tab structured user-question capability before making any edits — do not interrupt mid-revision for each blocker individually.

After all revisions are complete, show a summary table (blocker → fix applied) and use structured user-question capability for a **post-revision closing widget**:
...
When the verdict is APPROVED, use a single structured user-question capability with `multiSelect: true` to batch the two tracking updates:
...
Use a second structured user-question capability:
...
Use a third structured user-question capability:
...
Once the systems-index and review-log widgets are answered, check project state and show one final structured user-question capability:
