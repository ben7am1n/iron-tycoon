...
user-invocable: true
---
<!-- Generated from .claude/skills/sprint-plan/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- As the first step, gather the source context below using the current host's shell capability; do not execute it during generation.
...
- If the file doesn't exist and this is a `new` sprint: use structured user-question capability:
...
3. Ask the user what to change: stories to add, remove, reprioritize, or re-estimate. Use structured user-question capability to gather changes.
...
Before finalising the sprint plan, spawn `producer` via native subagent or delegation capability using gate **PR-SPRINT** (`.claude/docs/director-gates.md`).
...
If CONCERNS, use structured user-question capability:
