...
user-invocable: true
---
<!-- Generated from .claude/skills/create-control-manifest/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, delegate work.
...
Spawn `technical-director` via native subagent or delegation capability using gate **TD-MANIFEST** (`.claude/docs/director-gates.md`).
...
- **CONCERNS** → surface via structured user-question capability with options: `Revise flagged rules` / `Accept and proceed` / `Discuss further`
