...
user-invocable: true
---
<!-- Generated from .claude/skills/map-systems/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, ask structured questions, track plans, delegate work.
...
- Use structured user-question capability to ask:
...
Then use structured user-question capability to capture feedback:
...
Use structured user-question capability to ask: "Does this dependency ordering look right? Any
...
**After dependency mapping is approved, spawn `technical-director` via native subagent or delegation capability using gate TD-SYSTEM-BOUNDARY (`.claude/docs/director-gates.md`) before proceeding to priority assignment.**
...
Use structured user-question capability to ask: "Do these priority assignments match your vision?
...
**After priorities are approved, spawn `producer` via native subagent or delegation capability using gate PR-SCOPE (`.claude/docs/director-gates.md`) before writing the index.**
...
**After the systems index is written, spawn `creative-director` via native subagent or delegation capability using gate CD-SYSTEMS (`.claude/docs/director-gates.md`).**
...
Use structured user-question capability for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `/design-system` completes, use structured user-question capability:
...
After the systems index is created (or after designing some systems), present next actions using structured user-question capability:
