...
user-invocable: true
---
<!-- Generated from .claude/skills/gate-check/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, run shell commands, modify files, delegate work, ask structured questions.
...
**Target phase:** `invocation argument 0` (blank = auto-detect current stage, then validate next transition)
...
Before generating the final verdict, spawn all four directors as **parallel subagents** via native subagent or delegation capability using the parallel gate protocol from `.claude/docs/director-gates.md`. Issue all four Task calls simultaneously — do not wait for one before starting the next.
...
After the verdict is presented and any stage.txt update is complete, close with a structured next-step prompt using structured user-question capability.
