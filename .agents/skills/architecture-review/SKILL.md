...
user-invocable: true
---
<!-- Generated from .claude/skills/architecture-review/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Requested quality tier: high; reasoning effort: high.
- Preferred stable role: technical-director; use native delegation or role loading.
...
After completing the engine audit above, spawn the **primary engine specialist** via native subagent or delegation capability for a domain-expert second opinion:
...
If [B]: display the complete proposed systems-index section, then re-ask with structured user-question capability.
...
Use structured user-question capability for the write approval:
...
For `rtm` mode, use structured user-question capability:
...
Then close with structured user-question capability tailored to the pre-gate checklist state:
...
3. **Offer options** via structured user-question capability with three choices:
...
5. **Use structured user-question capability for write approvals** — plain text "May I?" is not
