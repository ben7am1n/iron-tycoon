...
user-invocable: true
---
<!-- Generated from .claude/skills/team-qa/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Preferred stable role: qa-lead; use native delegation or role loading.


When this skill is invoked, orchestrate the QA team through a structured testing cycle.

**Decision Points:** At each phase transition, use structured user-question capability to present
...
Spawn `qa-lead` via native subagent or delegation capability to review all in-scope stories and produce a QA strategy.
...
Present the qa-lead's full strategy to the user, then use structured user-question capability:
...
Spawn `qa-tester` via native subagent or delegation capability for each story (run in parallel where possible), providing:
- The story file path
...
Use structured user-question capability per story group (batched 3-4 at a time):
...
Batch stories into groups of 3-4 and use structured user-question capability for each:
...
After each FAIL result: use structured user-question capability to collect the failure description, then spawn `qa-tester` via native subagent or delegation capability to write a formal bug report in `production/qa/bugs/`.
...
Spawn `qa-lead` via native subagent or delegation capability to produce the sign-off report using all results from Phases 4–6.
