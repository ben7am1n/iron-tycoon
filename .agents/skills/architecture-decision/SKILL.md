...
user-invocable: true
---
<!-- Generated from .claude/skills/architecture-decision/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
a **confirm/adjust** prompt using structured user-question capability — not open-ended questions.
...
decisions remain, present each one as a separate structured user-question capability with the proposed
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via native subagent or delegation capability to validate the drafted ADR:
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` via native subagent or delegation capability using gate **TD-ADR** (`.claude/docs/director-gates.md`):
...
5. **Write approval** — Use structured user-question capability:
...
Ask using structured user-question capability:
...
After the ADR is written (and registry optionally updated), close with structured user-question capability.
