...
user-invocable: true
---
<!-- Generated from .claude/skills/setup-engine/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, search the web, read web pages, delegate work, ask structured questions.
...
**Question 1 — Prior experience** (ask this first, always, via structured user-question capability):
...
**Question 2 — Target platform** (ask this second, always, via structured user-question capability — platform eliminates or heavily weights engines before any other factor):
...
5. Use structured user-question capability to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use structured user-question capability with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
### Remaining Sections
- **Performance Budgets**: Use structured user-question capability:
