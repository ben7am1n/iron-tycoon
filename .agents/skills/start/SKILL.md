...
user-invocable: true
---
<!-- Generated from .claude/skills/start/SKILL.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, ask structured questions.
...
This is the first thing the user sees. Use structured user-question capability with these exact options so the user can click rather than type:
...
1. Ask them to describe their concept in one sentence — genre and core mechanic. Use plain text, not structured user-question capability (it's an open response).
2. Acknowledge the concept, then use structured user-question capability to offer two paths:
...
**If it does not exist**: Use structured user-question capability:
...
After presenting the recommended path, use structured user-question capability to ask the user which step they'd like to take first. Never auto-run the next skill.
