...
capabilities: ["read files", "discover files", "search text", "modify files", "run shell commands"]
---
<!-- Generated from .claude/agents/prototyper.md; run `python3 -m tools.platform_adapter.generate generate`. -->
...
- Allowed capability classes: read files, discover files, search text, modify files, run shell commands.
- Isolation preference: use a host-native isolated worktree when available, while preserving approval gates.
